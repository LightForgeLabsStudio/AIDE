#!/usr/bin/env bash
# Install AIDE skills for Qwen Code (repo-local .qwen/skills)

set -e

echo -e "\033[0;36mInstalling AIDE skills for Qwen Code...\033[0m"
...
SKILLS_PATH="$REPO_ROOT/.qwen/skills"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: install-qwen.sh [--symlink] [--repo-root PATH] [--skills-path PATH]"
            exit 1
            ;;
    esac
done
...
echo -e "\033[0;37mTarget Qwen skills dir: $SKILLS_PATH\033[0m"
...
echo ""
echo -e "\033[0;36mInstalled skills (repo-local .qwen/skills):\033[0m"
for skill_dir in "$AIDE_SKILLS_DIR"/*/; do
    if [ "$(basename "$skill_dir")" = "dist" ]; then
        continue
    fi
...
echo "  2. Type '/' in Qwen chat to see available commands"
echo ""
